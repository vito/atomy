...
describe Atomy::MessageStructure do
  ...
  context "when a word with a block" do
    let(:node) { ast("foo &blk") }
    its(:name) { should == :foo }
    its(:arguments) { should be_empty }
    its(:receiver) { should be_nil }
    its(:block) { should == ast("blk") }
  end

  context "when a word followed by ! with a block" do
    let(:node) { ast("foo! &blk") }
    its(:name) { should == :foo! }
    its(:arguments) { should be_empty }
    its(:receiver) { should be_nil }
  ...
  context "when a word with arguments with a block" do
  ...
  context "when a word followed by ! with arguments with a block" do
    let(:node) { ast("foo!(a, b) &blk") }
    its(:name) { should == :foo! }
    its(:arguments) { should == [ast("a"), ast("b")] }
    its(:receiver) { should be_nil }
  ...
  context "when a node followed by a word with a block" do
  ...
  context "when a node followed by a word followed by ! with a block" do
    let(:node) { ast("42 foo! &blk") }
    its(:name) { should == :foo! }
    its(:arguments) { should be_empty }
    its(:receiver) { should == ast("42") }
  ...
  context "when a node followed by a word with arguments with a block" do
  ...
  context "when a node followed by a word followed by ! with arguments with a block" do
    let(:node) { ast("42 foo!(a, b) &blk") }
    its(:name) { should == :foo! }
    its(:arguments) { should == [ast("a"), ast("b")] }
    its(:receiver) { should == ast("42") }
