...
module Atomy::Patterns
  ...
  def self.instantiate(cls)
    args = []

    cls.children[:required].each do
      args << random_pattern
    end

    cls.children[:many].each do
      val = []
      rand(5).times do
        val << random_pattern
      end
    ...
    cls.attributes[:required].each do
      args << random_symbol
    end

    cls.attributes[:many].each do
      val = []
      rand(5).times do
        val << random_symbol
      end
    ...
    cls.slots[:required].each do
      args << rand(16)
    end

    cls.children[:optional].each do
      if rand(2) == 0
        args << nil
      else
        args << random_pattern
    ...
    cls.attributes[:optional].each do
      if rand(2) == 0
        args << nil
      else
        args << random_symbol
  ...
  def self.random_pattern
    instantiate(PATTERN_TYPES.sample)
  end
